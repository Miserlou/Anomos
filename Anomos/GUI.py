...
def getText():
	...
	dialog = gtk.MessageDialog(
		...
		gtk.BUTTONS_OK,
		None)
	dialog.set_markup('Please enter the <b>magnet URI</b>:')
	...
	hbox = gtk.HBox()
	hbox.pack_start(gtk.Label("Magnet:"), False, 5, 5)
	hbox.pack_end(entry)
	#some secondary text
	dialog.format_secondary_markup("This will be anonymized with the anonymizer URL")
	#add it and show it
...
def getTorrentString():
	...
	dialog = gtk.MessageDialog(
		...
		gtk.BUTTONS_OK,
		None)
	dialog.set_markup('Please enter the <b>torrent string</b>:')
	#create the text input field
	entry = gtk.Entry()
	#allow the user to press enter to do ok
	entry.connect("activate", responseToDialog, dialog, gtk.RESPONSE_OK)
	#create a horizontal box to pack the entry and a label
	hbox = gtk.HBox()
	hbox.pack_start(gtk.Label("Torrent:"), False, 5, 5)
	hbox.pack_end(entry)
	#some secondary text
	dialog.format_secondary_markup("This will be anonymized with the anonymizer URL")
	#add it and show it
